...
class SparkfunListener(MessageListener):
    """
    MessageListener that posts messages to data.sparkfun.com.

    The Sparkfun info (uri, api key, etc) is configured on a device-by-device
    ...
    def handle(self, message):
        ...
        sparkfun_config = self.lookup_config(message.source, message.type)
        datastream = self.get_datastream(sparkfun_config)

        # Allow configuration of which event data field to post. Default is "value"
        field_name = sparkfun_config.get("field", "value")
